...
// void separate(){
//    char            *mp4_filename     =
//    "/Users/a1234/Desktop/workspace/littleGif/assets/testVideo.mp4"; //
//    mp4视频，200kbps.768x320
//        char            *h264_filename    = "out.h264";                 //
//        视频流输出文件
////        char            *aac_filename     = "out.aac";                  //
///音频流输出文件 /        FILE            *aac_fd           =
///fopen(aac_filename, "wb");  // 打开aac音频输出文件
//        FILE            *h264_fd          = fopen(h264_filename, "wb"); //
//        打开h264视频输出文件 AVFormatContext *fmt_ctx          =
//        avformat_alloc_context();   // 解封装上下文 AVPacket        *pkt =
//        av_packet_alloc();          // 为packet分配空间，用于存放音视频帧数据
//        AVBSFContext    *bsf_ctx          = NULL;                       //
//        bit流过滤器上下文 int             video_index       = -1; //
//        视频流索引 int             audio_index       = -1; // 音频流索引 const
...
//
//
//        int ret = avformat_open_input(&fmt_ctx, mp4_filename, NULL, NULL); //
//        打开音视频流 if(ret < 0) {
...
//
//        // 获取视频和音频索引
//        video_index = av_find_best_stream(fmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1,
//        NULL, 0); audio_index = av_find_best_stream(fmt_ctx,
//        AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0); if(video_index == -1 ||
//        audio_index == -1) {
//            avformat_close_input(&fmt_ctx); // 关闭音视频流
//            return -1; // 音视频索引获取失败
//        }
...
//        fmt_ctx->streams[video_index]->codecpar); if(ret < 0) {
//            avformat_close_input(&fmt_ctx); // 关闭音视频流
//            av_bsf_free(&bsf_ctx);
//            return -1; // 参数拷贝失败
//        }
...
///每个packet都要添加一个7字节的音频头 / adts_header(adts_header_buf, pkt->size,
